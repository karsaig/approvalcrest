...
|---|---|---|---|---|
| `approvalcrestReflection` | `aCReflection` | `safe` | Field access mode: `safe` (auto-opens modules; on JDK 26+ that needs the approvalcrest agent), `force` (requires --add-opens), or `fallback` (getter-based). | [field-access-modes](field-access-modes.md) |
