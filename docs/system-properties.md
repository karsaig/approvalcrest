...
| `fileMatcherSourceRoot` | `fmSourceRoot` | `src/test/java` | Base test source directory (relative to project root) that approved files are stored under. Set to e.g. `src/it/java` or `src/test/kotlin` when tests live outside the standard directory. | [file-control](file-control.md) |
