...
| `fileMatcherSourceRoot` | `fmSourceRoot` | `src/test/java` | Base test source directory (relative to project root) that approved files are stored under. Set to e.g. `src/it/java` when tests live outside the standard directory. Not honoured by the Kotlin matchers, which hardcode `src/test/kotlin`. | [file-control](file-control.md) |
