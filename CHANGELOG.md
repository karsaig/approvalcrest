...
-----
- An ordering matcher written with the wrong kind of number now reports a mismatch instead of throwing `ClassCastException`. `.with("someLongField", greaterThan(0))` matches nothing, because a whole number reaches the matcher as a `Long` and Hamcrest compares only within one boxing — that part is unchanged and intended. What was broken is what happened next: the failure message was built by casting the value without that check, so the assertion died with a stack trace naming Hamcrest internals rather than the field. Wrapping the matcher in `allOf`, `both().and()`, `hasItem` or `everyItem` was worse again — those call the inner matcher while deciding, so the exception escaped before any verdict was reached. The message now names the field, the value, the type it arrived as and the cast that failed, and says to write the matcher's number in the same form as the value's. Reachable from any ordering matcher over a whole number: always for a `long`-valued field, and for the name-pattern forms, which read the serialised output only. See [custom-matching](docs/custom-matching.md).
- Added `.alsoCheck(path, matcher)` and `.alsoCheckMatching(fieldNamePattern, matcher)`, which apply a custom matcher to a field *as well as* comparing it, where `.with(...)` and `.withMatcher(...)` apply one *instead of* comparing it. The existing pair takes the matched field out of the comparison, so it never reaches the approved file and a value that later drifts cannot fail — right for a generated id, wrong for a value you can pin down but want a stronger guarantee about. Both existing methods are unchanged and no approved file is affected. Switching an existing `.with(...)` call to `.alsoCheck(...)` does need that approved file regenerating, since it has never held the field: re-run with `-DfileMatcherUpdateInPlace=true`, which rewrites the file as long as the new matcher passes. Anyone implementing `CustomisableMatcher` themselves must add the two methods; callers need no change. See [custom-matching](docs/custom-matching.md).
