...
- Added test coverage for `List<Map<String,Object>>` inputs: `mixedFeaturesCollectionTest` and `mixedFeaturesCollectionTwoTest` verify that path-based and pattern-based custom matchers both work when the actual value is a list of maps. `pathAbsentFromAllElementsThrowsException` verifies that the lenient fanout does not swallow genuine "field not found" errors — the `IllegalArgumentException` is still thrown when the path is absent from every element across all four input types (Map object, Map JSON string, List-of-Maps object, List-of-Maps JSON string).
