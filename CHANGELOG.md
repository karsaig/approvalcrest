...
- Fixed a `StackOverflowError` when comparing a graph of `Throwable`s that reference each other in a cycle. Cyclic ordinary beans were already handled correctly.
...
- Object-graph reference ids containing hexadecimal letters are now recognised consistently. From the tenth object in a graph onwards an id contains the letters `a`-`f`, which the throwable adapter did not accept.
...
- Fixed `Optional<Interface>` and `Optional<AbstractClass>` serialising as an empty `{}`. Where the declared type parameter is an interface or abstract class, the runtime type is used instead. See [supported-types](docs/supported-types.md).
...
- `approvalcrest-junit-jupiter` supports JUnit Platform 6.0.3 on Java 17, 21 and 25. See [junit5-jupiter](docs/junit5-jupiter.md).
...
- Fixed ignoring a path through an array erroring on primitive elements.
- Ignoring all fields of a bean `Map` key now removes the whole entry cleanly.
- Ignoring a field inside a `Map` value no longer leaves an orphaned empty array.
