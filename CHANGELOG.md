...
- Documented two limitations that had been folklore. A map key spelled like a circular-reference wrapper key — `0x` followed by lowercase hex — is skipped during path navigation as if it were one, so a path that omits such a key still reaches through it, where omitting any other key matches nothing. And a map whose keys are not strings, primitives or enums is written as `[key, value]` pairs which the sort reorders, so the key/value direction is not recorded: such a map and its transpose produce the same bytes, and an approved file for one is matched by the other. Neither is fixable without changing what gets written, which would invalidate existing approved files, so both are recorded and pinned by tests rather than changed. See [ignoring-fields](docs/ignoring-fields.md) and [sorting](docs/sorting.md).
