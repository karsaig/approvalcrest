...
-----
- An ordering matcher written with the wrong kind of number no longer kills the test with a `ClassCastException`. `.with("someLongField", greaterThan(0))` matches nothing, because a whole number reaches the matcher as a `Long` and Hamcrest compares only within one boxing — that part is unchanged. What was broken is what came next: the failure message was built without that check, so the assertion died with a stack trace naming Hamcrest internals rather than the field. Wrapping the matcher in `allOf`, `both().and()`, `hasItem` or `everyItem` was worse, since those decide by calling the inner matcher — that shape threw before any verdict existed, and over an `int` field it now passes, which is the one case where a result changes rather than a message. The message names the field, the value, the type it arrived as and the cast that failed. Watch out for one combination that still cannot fail: `not(greaterThan(...))` and its counterparts pass whatever the data holds when the boxings differ. See [custom-matching](docs/custom-matching.md).
- Added `.alsoCheck(path, matcher)` and `.alsoCheckMatching(fieldNamePattern, matcher)`, which apply a custom matcher to a field *as well as* comparing it, where `.with(...)` and `.withMatcher(...)` apply one *instead of* comparing it. The existing pair takes the matched field out of the comparison, so it never reaches the approved file and a value that later drifts cannot fail — right for a generated id, wrong for a value you can pin down but want a stronger guarantee about. Both existing methods are unchanged and no approved file is affected. Switching an existing `.with(...)` call to `.alsoCheck(...)` does need that approved file regenerating, since it has never held the field: re-run with `-DfileMatcherUpdateInPlace=true`, which rewrites the file as long as the new matcher passes. Anyone implementing `CustomisableMatcher` themselves must add the two methods; callers need no change. One side effect for existing tests: with `.with(...)` and machine-readable output together, the order of the `ignoredFields` array can differ, though its entries and every other field are unchanged. See [custom-matching](docs/custom-matching.md).
