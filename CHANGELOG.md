...
-----
- A class that inherits fields from a locked JDK module — most commonly your own exception class, which extends `RuntimeException` and so carries `Throwable`'s private fields — no longer fails to serialise on a JDK where those fields cannot be reached. It threw `JsonIOException: Failed making field 'java.lang.Throwable#detailMessage' accessible` instead of falling back to getter-based output the way a locked type itself does, because the class's own module is not locked and nothing claimed it. Only affects runs where no module can be opened at all, which today means JDK 26 or `fallback` mode; every other run is unchanged.
- On JDK 26 and later, types from locked JDK modules (`java.time.*`, `java.lang.*`, `java.math.*`, etc.) now serialise via their public getters instead of coming out as an empty `{}`. JDK 26 disables `sun.misc.Unsafe` rather than removing it, so the default safe mode kept using it, read nothing, and produced an object with no fields — an approved file could match `{}` and report a pass having compared nothing. Safe mode now checks that Unsafe can still read a field before relying on it, and falls back to getter-based output when it cannot, as it already did on a JDK with no Unsafe at all. JDK 8 to 25 are unaffected and their approved files are unchanged. On JDK 26, approved files for locked-module types need regenerating with `-DfileMatcherUpdateInPlace=true`; to keep the old field-based output instead, run with `--sun-misc-unsafe-memory-access=allow` or use force mode with `--add-opens`. See [field-access-modes](docs/field-access-modes.md).
...
- Ignoring a wildcard path can remove the container itself. If the ignored field is the only one on each map value, every value empties, so each entry goes, then the array, then the map field — keys included. A named key scopes that to one entry, whereas `*` applies to every entry at once, which makes whole-field disappearance the common case rather than the corner. Check what the comparison still covers rather than reading a pass as proof the map was compared.
