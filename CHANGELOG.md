...
- A `*` path segment now means "every named child at this position" — a map key, an object property or a bean field — so `.with("ordersByRef.*.trackingCode", notNullValue())` and `.ignoring("ordersByRef.*.generatedId")` apply to every entry of a map. Previously the only way to reach a map's values was to name each key. `*` is a wildcard only in a non-final segment: as the last segment it keeps its existing meaning, a key literally named `*`, so a document or `Map<String,?>` that has one is unaffected. Collections and arrays need no wildcard — they are already traversed transparently, so `orders.trackingCode` already means "in every order" and `orders.*.trackingCode` is a different query, not a synonym. Every child the path resolves against must satisfy the matcher, but children that do not carry the rest of the path are skipped, so a heterogeneous map narrows what you asserted; a path that resolves against nothing at all is an error for `.with(...)` and a no-op for `.ignoring(...)`. See [custom-matching](docs/custom-matching.md) and [ignoring-fields](docs/ignoring-fields.md).
- Ignoring a wildcard path can remove the container itself. If the ignored field is the only one on each map value, every value empties, so each entry goes, then the array, then the map field — keys included. A named key scopes that to one entry; `*` hits every entry at once, which makes whole-field disappearance the common case rather than the corner. Check what the comparison still covers rather than reading a pass as proof the map was compared.
- `sortField(...)` accepts the same `*` segment, so a sort path can reach the collection under every map entry instead of naming each key. A wildcard and a literal key at the same level both apply rather than one overriding the other, and a `SortField`'s ignored sub-paths accept `*` too, so a field can be kept out of the sort key under every child. Previously a `*` there was read as a literal key name, so nothing matched and the field silently stayed in the key — baking an order nobody asked for into the approved file. See [sorting](docs/sorting.md).
...
- A field path can now cross a `Set`- or `Map`-typed field when it resolves against the serialised JSON rather than against the object — which is every path that goes deeper than the field itself, since the object walk has no `Map` branch. Such a field's JSON key carries an internal prefix, and the JSON path resolver did not account for it, so `.with("ordersByRef.A-1.trackingCode", …)` failed with `ordersByRef.A-1.trackingCode does not exist` however well-formed it was. Addressing a map entry by its key now works for `.with(...)` as it already did for `.ignoring(...)` and `.sortField(...)`. Skipping the key still does not reach the values: a map is traversed by key, not fanned out over. See [custom-matching](docs/custom-matching.md).
- Ignoring a field inside a circular-reference graph now reaches every object in that graph. The walk descends into the graph's objects looking for the path, but stopped at the first object it changed, so the same field survived in the others. Because the walk runs separately over the actual value and the approved content, the two ended up filtered differently and the assertion failed on the data rather than on the ignored field — the failure pointed at the wrong thing. Only graphs holding more than one object carrying the ignored path were affected.
- A custom matcher that the object under comparison already settles is no longer re-evaluated against the serialised JSON. `.with(path, matcher)` resolves the path against the object first and retries against the JSON only to cover what the object walk cannot reach — an array segment, a raw JSON string input, or a number whose JSON form is a `Long` where the field is an `int`. The retry also ran when the path resolved cleanly and the matcher simply rejected the value, and a collection or map matcher cannot match the `JsonArray` the JSON form supplies, so its negation came back true and the failure was discarded. `.with("orders", not(empty()))` therefore passed on an empty collection, as did `not(hasSize(n))`, `not(hasItem(x))`, `not(contains(...))` and `not(containsInAnyOrder(...))` — assertions that could never fail. Such a matcher now reports its mismatch. One combination is not fixed by this and cannot be: `not(hasItem(x))`, `not(contains(...))` and `not(containsInAnyOrder(...))` still pass whatever the data holds when the input is a raw JSON string, the elements are objects rather than scalars, and the element matcher is written against the element's own class — there is no element class in parsed text for such a matcher to match, and the same `JsonArray` is handed to container matchers, which do work here, so the two cannot be told apart. `not(empty())` and `not(hasSize(n))` are fixed for both input forms, and all of them work over scalar elements. Paths that fan out through a collection are unaffected: they target leaf values, not the container, so numeric coercion still applies. See [custom-matching](docs/custom-matching.md).
...
- **[breaking]** A map holding two entries whose key and value both serialise identically now renders each entry as its own key/value pair. Such entries were grouped together, and the whole group rendered into a single pair array, so two of them serialised as two copies of one four-element array rather than as two pairs — an approved file that misrepresented the map. Both positions held the same array instance, so a walk over the map visited those objects twice. Only maps whose keys are not primitives, `String`s or enums are affected, and only where two entries collide; every other map is byte-identical, so no approved file in this repository changed. An approved file that recorded the duplicated shape will now fail. **Migration:** re-run tests with `-DfileMatcherUpdateInPlace=true` to regenerate the affected approved files.
- **[breaking]** Ignoring everything under a `Set`- or `Map`-typed field now removes the field, rather than leaving an empty collection behind in the approved file. Such a field's JSON key carries an internal sorting prefix that is stripped again before the file is written, and the emptied field was removed by its unprefixed name, so nothing went: the file kept `"ordersByRef": []` where an ordinary field emptied the same way disappeared. The prefix decided the outcome while being invisible in the result. Only fields that ignoring empties completely are affected, and an unmarked field in the same position already disappeared, so this is parity rather than a new rule. Strict matching is on by default, so an existing approved file that still holds the empty collection will now fail. **Migration:** re-run tests with `-DfileMatcherUpdateInPlace=true` to regenerate the affected approved files.
- **[breaking]** Ignoring a field inside an object held in a collection no longer removes the other values in that collection. Where the ignored field was the object's only one, the emptied object went — correctly — and anything sitting beside it went with it, so `.ignoring("list.a")` over `{"list":[{"a":1},"keep-me",42]}` left the collection empty and then dropped the field altogether. Two values the rule never mentioned disappeared, and with both sides of the comparison filtered alike the assertion still passed. That clearing exists for maps with a bean key, where a key emptied by ignoring leaves its value with nothing to belong to, and it now applies only to things shaped like such an entry: an array of exactly two elements that is itself inside an array. A collection held by a field is neither, and nor is a nested collection of any other length. A nested collection of exactly two elements still cannot be told apart from a map entry, so one reduced to plain values is still emptied, and the field still goes if nothing else remains in it. An approved file generated while those values were being dropped will now fail, because they are present again. **Migration:** re-run tests with `-DfileMatcherUpdateInPlace=true` to regenerate the affected approved files.
- Machine-readable output now lists an ignore rule that took effect below the top of its path, and lists such a rule once rather than twice. The filter reported only whether it had removed a direct child of the object it was called on, so `.ignoring("a.b.c")` went unrecorded whenever `a.b` kept its other fields — the ordinary case rather than an edge one. Separately, one tracker records both the run over the actual value and the run over the approved content, so a rule matching both was counted twice. Only rule paths are deduplicated: an element index, or the path of a parent removed for becoming empty, identifies a location rather than a rule, and sibling locations can still share a path, so collapsing those would lose a real removal instead of a duplicate. This affects the machine-readable report only; what gets compared is unchanged.
- Documented a limitation that had been folklore. A map key spelled like a circular-reference wrapper key — `0x` followed by lowercase hex — is skipped during path navigation as if it were one, so a path that omits such a key still reaches through it, where omitting any other key matches nothing. It is not fixable without changing how wrappers are written, which would invalidate every approved file containing a circular reference, so it is recorded and pinned by tests rather than changed. See [ignoring-fields](docs/ignoring-fields.md).
- **[breaking]** A map whose keys are not primitives, `String`s or enums now records which half of an entry was the key. Such a map is written as `[key, value]` pairs, and the sort recursed into that pair like any other array and reordered it by its JSON — so the key/value direction was lost: `{a: z}` and `{z: a}` produced the same bytes, and an approved file written for one was matched by the other. The pair is now left in place, while both halves are still sorted, so an entry with a collection-valued half is unaffected. The order of *entries* can change too, where some entries were reordered and others were not. For a file matcher this applies under strict file matching, which is the default; with it off nothing changes, because there the approved content is sorted as well and cannot tell a pair from a nested collection. The written form therefore depends on that setting, so switching it in either direction needs the affected files regenerating. `sameBeanAs` serialises both sides itself, so it is affected unconditionally and there is no setting to turn this off and no file to regenerate: an assertion that passed only because a map and its transpose compared equal now fails, and has to be rewritten. A map at the root is covered too, recognised from the object's own type. A map reached without a field name and not at the root — as another map's value, a collection element, or through an `Object`-declared field — is unprotected: its pairs are reordered if anything sorts at that level, and left alone if nothing does. A raw JSON string input is unchanged. No approved file in this repository changed. **Migration:** re-run tests with `-DfileMatcherUpdateInPlace=true` to regenerate approved files holding such a map. See [sorting](docs/sorting.md).
- Corrected a wrong entry in the released 1.0.1 notes. It claimed that sorting an array-of-arrays preserves inner array order; inner arrays are sorted together with the outer one, and no release has behaved otherwise. The entry is annotated in place rather than rewritten, so anyone who followed its migration advice can see that it was never needed.
- A map with a `null` key no longer aborts the comparison. It threw a `NullPointerException` before any file was written — and whether it threw depended on what the map's other keys rendered as, so the same shape failed or worked by accident. The key is now recorded as the member name `"null"`, in a map whose other keys are primitives, `String`s or enums; where any other key forces the `[key, value]` form, it is written as a bare JSON null in the entry, as any key is there. Note that a `null` key and a `String` key of `"null"` are then indistinguishable: both entries are recorded, but a comparison between one map and the other reports a match. Rendering the key so it could not collide would put it at a position path navigation skips, silently stopping `.ignoring("map.someKey.leaf")` from resolving for every other key in the map. See [sorting](docs/sorting.md).
...
- Fixed a `StackOverflowError` when comparing a graph of `Throwable`s that reference each other in a cycle. The walk that names the concrete type of each nested throwable follows object-graph references, which jump across the graph rather than descending the serialised tree, so a cycle led back to a reference already visited and recursed until the stack ran out. Cyclic ordinary beans were already handled correctly.
...
- Test methods inherited from an **abstract** base class now fail with a clear error instead of silently sharing one approved file. The stack-trace route (and TestNG's injected `Method`) can only report the class a method is *declared* in, so every subclass resolved to the same file and overwrote the others. Pass `TestInfo` (JUnit 5), use `Junit4DesciptionWatcher` (JUnit 4), or supply a custom `TestMetaInformation` to get one approved file per subclass. Concrete test classes are unaffected.
...
- Object-graph reference ids containing hexadecimal letters are now recognised consistently. Ids are generated with `Integer.toHexString`, so from the tenth object in a graph onwards they contain `a`-`f`, but the throwable adapter matched them against a decimal-only pattern. The two places that parse this format now share one definition.
...
- Added AI tip to the type-mismatch failure message (`sameBeanAs` with incompatible types): the non-machine-readable branch now ends with the standard `[AI tip] Re-run with fmAI=true for structured, machine-readable output.` tip, consistent with content-mismatch failures. Tests verify the concrete tip text and that machine-readable `TYPE_MISMATCH` output is valid JSON without an AI tip.
...
- **[breaking]** Fixed sorting of arrays-of-arrays: only the outer array is reordered; inner array element order is preserved. Previously `sortField("groups")` on a `groups: [[Z,B],[A,C]]` also sorted the inner arrays into `[[A,C],[B,Z]]`; now the inner arrays stay as-is and only the outer order changes. **Migration:** if inner array order matters, switch the inner collection type to `Set` (auto-sorted by type) or add an explicit `sortField("innerFieldName")` for the inner field. See [sorting](docs/sorting.md). **[Correction, 1.5.1:** this entry is wrong, and no release has behaved as it describes. Inner arrays are sorted together with the outer one whenever the outer array's sort is configured. The migration advice above was therefore unnecessary, though harmless — switching to a `Set` or adding an inner `sortField` both still sort. See [sorting](docs/sorting.md), which has always been accurate.**]
